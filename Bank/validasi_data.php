...
<h2 class="sub-header">Data Mahasiswa</h2>
          <div class="module">
            ...
            $dbp = mysql_query("SELECT * FROM user "); 
            $no = 1;
            while ($hdbp = mysql_fetch_array($dbp)) {
              ?>
                <tr>
                  ...
                  <td><?php echo $hdbp['status'] ?></td>
                  <?php 
                    if ($hdbp['status'] == 'aktive') {
                      ?>
                        <td><a href="index.php?model=validasi&mode=non&id=<?php echo $hdbp['id_user']; ?>" class="btn btn-primary">Non Aktifkan</a></td>
                      <?php
                    }else{
                      ?>
                        <td><a href="index.php?model=validasi&mode=aktive&id=<?php echo $hdbp['id_user']; ?>" class="btn btn-primary">Aktifkan</a></td>
                    <?php } ?>
